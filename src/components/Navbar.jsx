...
const links = [
  { label: 'טלוויזיות', href: '#tv' },
  { label: 'שמע', href: '#audio' },
  { label: 'מחשבים', href: '#computers' },
            ...
            <nav className="container-luxe py-6 flex flex-col gap-4">
              {links.map((l) => (
                <a key={l.href} href={l.href} onClick={() => setOpen(false)}
                   className="text-lg font-medium text-white/80 hover:text-gold-300 py-2 border-b border-white/5">
                  {l.label}
                </a>
              ))}
            </nav>
